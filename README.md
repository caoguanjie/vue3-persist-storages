...
它们之间各自有相应的API，存在差异性比较大，在实际业务开发中，几乎有可能几种存储方案都会使用，他们各自有各自的优缺点，想要进一步了解的，请参考文章：[数据持久化存储方案](https://caoguanjie.github.io/fitsadmin-docs/guide/persist-storages.html)，这里不做过多赘叙，在相同的产品中，对每个持久化存储的方式都有相同需求，于是就有了`vue3-persist-storages`的出现，目标是就是创造相同api，封装相同的功能，兼容各种实际场景，抹平不同存储的差异化调用。
